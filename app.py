...
app.config['MAIL_PASSWORD'] = 'zatrwjdvmhhxfxrd'
...
def handle_getrawgraphfile(caseid, projectid, userid):
    if request.method == 'OPTIONS':
        ...
        if (addon == "pulseInstall"):
            # TODO: Add pulse install directory
...
def send_email():
    ...
    msg.body = f'New user has signed up\n{event["user"]["email"]}\nhttps://manage.auth0.com/dashboard/us/undex-fspi/users \nPlease send the user verification message: simsolver.epitomeri.com:4001/verify?email={event["user"]["email"]}'
    mail.send(msg)
    return "Message sent!"
